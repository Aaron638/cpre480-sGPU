...
/*****************************************************************************
 * 04_wake me up inside - translates and scales a triangle in the vertex shader
 *
 *
 * NOTES:
...
int main( void ) {
	...
	while( glfwGetKey(window, GLFW_KEY_ESCAPE) != GLFW_PRESS && glfwWindowShouldClose(window) == 0 ) {
		...
		printf("xoffset should be %d)\n", xoffset);
		printf("yoffset should be %d)\n", yoffset);
		printf("xscale should be %d )\n" , xscale);
		printf("yscale should be %d )\n" , yscale);

		// I have no idea if this is right
		printf("\x1B[32m  0x%08x\n",SGP_read32(SGPconfig, SGP_graphicsmap[SGP_UNIFORMS+0].baseaddr);
		printf("\x1B[32m  0x%08x\n",SGP_read32(SGPconfig, SGP_graphicsmap[SGP_UNIFORMS+1].baseaddr);
		printf("\x1B[32m  0x%08x\n",SGP_read32(SGPconfig, SGP_graphicsmap[SGP_UNIFORMS+2].baseaddr);
		printf("\x1B[32m  0x%08x\n",SGP_read32(SGPconfig, SGP_graphicsmap[SGP_UNIFORMS+3].baseaddr);
