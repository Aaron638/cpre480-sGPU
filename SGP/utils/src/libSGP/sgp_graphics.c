...
SGP_graphicsmap_t SGP_graphicsmap[SGP_GRAPHICS_NUMCOMPONENTS] = {
	...
	[SGP_VIEWPORT] 		= {0x44A80000, 0x44A8FFFC, "SGP_VIEWPORT      ", "Viewport transformation config        ", SGP_GRAPHICS_SYSTEM_OFFSET, SGP_AXI_VIEWPORT_DEBUG, SGP_GRAPHICS_NO_STATUS, SGP_GRAPHICS_NO_PCOUNT},
	[SGP_RENDER_OUTPUT] = {0x44A90000, 0x44A9FFFC, "SGP_RENDER_OUTPUT ", "Render Output (ROP) config            ", SGP_GRAPHICS_SYSTEM_OFFSET, SGP_AXI_RENDEROUTPUT_DEBUG, SGP_GRAPHICS_NO_STATUS, SGP_GRAPHICS_NO_PCOUNT},
	[SGP_RASTERIZER] 	= {0x44AA0000, 0x44AAFFFC, "SGP_RASTERIZER    ", "Rasterization unit config             ", SGP_GRAPHICS_SYSTEM_OFFSET, SGP_AXI_RASTERIZER_DEBUG, SGP_AXI_RASTERIZER_STATUS, SGP_GRAPHICS_NO_PCOUNT},
	[SGP_VERTEXSHADER] 	= {0x44AB0000, 0x44ABFFFC, "SGP_VERTEXSHADER  ", "Vertex shader control and config      ", SGP_GRAPHICS_SYSTEM_OFFSET, SGP_AXI_VERTEXSHADER_DEBUG, SGP_AXI_VERTEXSHADER_STATUS, SGP_AXI_VERTEXSHADER_RTCOUNTER},
...
void SGP_glxSwapBuffers(uint32_t flag)
{
	...
	if (flag & SGP_SYSTEM_WAITIDLE)
	{
		...
		if (SGPconfig->driverMode & SGP_ETH)
		{
			uint32_t i = 0;
			uint32_t idle;
			uint32_t vtxFetchStatus, vtxShaderStatus, vpStatus, rastStatus, fragShaderStatus, renderOutStatus;

			do
			{
				vtxFetchStatus 	= SGP_read32(SGPconfig, SGP_graphicsmap[SGP_VERTEX_FETCH].baseaddr 	+ SGP_AXI_VERTEXFETCH_STATUS);
...
void SGP_print_pc_regs()
{
	...
	for (int i = 0; i < SGP_GRAPHICS_NUMCOMPONENTS; i++)
	{
		if (SGP_graphicsmap[i].debug_register != SGP_GRAPHICS_NO_PCOUNT)
		{
			printf("\x1B[32m   %s     0x%08x\n", SGP_graphicsmap[i].name, SGP_read32(SGPconfig, SGP_graphicsmap[i].baseaddr + SGP_graphicsmap[i].pcount_register));
		}
	}
	printf("\x1B[0m\n");
...
void SGP_print_statusregs()
{
	...
	for (int i = 0; i < SGP_GRAPHICS_NUMCOMPONENTS; i++)
	{
		if (SGP_graphicsmap[i].debug_register != SGP_GRAPHICS_NO_STATUS)
		{
